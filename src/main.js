...
loop = function() {
  ...
  // console.log(Math.abs(player.x_cam + player.x))

  // if (player.x_cam + player.x > 800 && controller.right) {
  //   player.x_cam -= player.x_velocity + Math.abs(player.x_cam + player.x) - 800
  // }
  //
  // if (player.x_cam + player.x < 200 && controller.left) {
  //   player.x_cam -= player.x_velocity
  // }

  if (player.x > CanW - player.x_cam - delta) {
    player.x_cam = CanW - player.x - delta;
  }

  if (-player.x_cam + CanW > gameWidth) {
    player.x_cam = CanW - gameWidth
  }

  if (player.x < delta - player.x_cam) {
    player.x_cam = delta - player.x;
  }
  if (player.x_cam > 0) {
    player.x_cam = 0
  }
